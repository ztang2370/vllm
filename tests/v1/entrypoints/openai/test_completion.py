...
async def server_logic_bundle(args):
...
async def test_single_completion(server_logic_bundle,
                                 client: openai.AsyncOpenAI,
                                 register_completion, model_name: str):
    handle_request = make_handle_request_no_streaming(server_logic_bundle)
...
async def test_too_many_completion_logprobs(
    ...
    stream = await server_logic_bundle.create_completion(completion_request)
    error_found = False
...
async def test_batch_completions(server_logic_bundle,
                                 client: openai.AsyncOpenAI,
                                 register_completion, model_name: str):
    handle_request = make_handle_request_no_streaming(server_logic_bundle)
    ...
    for prompts in (["Hello, my name is"] * 2, [[0, 0, 0, 0, 0]] * 2):
        ...
        batch = await server_logic_bundle.create_completion(completion_request)
        texts = [""] * 2
